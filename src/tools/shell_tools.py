...
def is_safe_command(command: str) -> bool:
    # Если включен raw shell, считаем всё безопасным (на страх и риск пользователя)
...
async def shell_readonly(command: str) -> Dict[str, Any]:
    """Runs a shell command. Whitelist is bypassed if enable_raw_shell is True."""
    # Если raw_shell выключен, проверяем по жесткому списку
    if not config.enable_raw_shell:
        ...
        if not matched:
             raise MCPError(ErrorCode.TOOL_DISABLED, f"Command '{command}' is not in the read-only whitelist and raw shell is disabled.")

    if not is_safe_command(command):
        raise MCPError(ErrorCode.TOOL_DISABLED, f"Command '{command}' contains explicitly denied patterns.")
...
async def shell_privileged(command: str, confirm_dangerous: bool = True) -> Dict[str, Any]:
    """Runs a shell command via rish. No restrictions when enable_raw_shell is True."""
    if not config.enable_raw_shell:
        raise MCPError(ErrorCode.TOOL_DISABLED, "Privileged shell is disabled in configuration.")
    
    # Мы всё еще оставляем флаг в сигнатуре для совместимости, но не блокируем выполнение
    if not is_safe_command(command):
        raise MCPError(ErrorCode.TOOL_DISABLED, f"Command '{command}' contains explicitly denied patterns.")
